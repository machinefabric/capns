...
Source: `capdag/src/planner/live_cap_fab.rs`.
...
Each result is a `ReachableTargetInfo` containing the reachable planner state:

- target media URN
- target `is_sequence` cardinality
- path statistics for that exact state

### Path Finding

`find_paths_to_exact_target()` returns one or more `Strand` objects connecting a specific source state to a specific target state. The planner state is `(media_urn, is_sequence)`, so exact-target matching requires both the media URN and the target cardinality to match.
...
`Cap` nodes do the actual work. The other types handle data routing: `InputSlot` and `Output` are the plan's boundary; `ForEach`/`Collect` handle list processing.
...
The plan builder converts a `Strand` (path through the capfab) plus argument specifications into a `MachinePlan`.
