...
Connection setup between host and cartridge: frame exchange, limit negotiation, identity verification.
...
The host always sends first. The cartridge waits for the host's Hello before responding.
...
The cartridge's Hello includes a JSON-encoded manifest in `meta["manifest"]` as a byte string. The host's Hello does not include a manifest — it only proposes limits.
  ...
  "cap_groups": [
    {
      ...
      "caps": [
        {
          "urn": "cap:in=\"media:text;encoding=utf8\";out=\"media:text;encoding=utf8\";generate;model=gguf",
...
The manifest is required — a cartridge Hello without a manifest causes a handshake error. The host extracts the manifest and uses it to build the capability routing table.

After the host receives the manifest, it validates that `CAP_IDENTITY` (`cap:in="media:";out="media:"`) is present. Every cartridge must declare the identity capability because the handshake verifies it immediately after the Hello exchange.
...
The cartridge's identity handler receives the nonce and echoes it back through the same streaming pattern. The host reads the response, decodes the CBOR chunks, concatenates the bytes, and compares them to the original nonce.
...
Sends the host's Hello, reads the cartridge's Hello, negotiates limits, updates both reader and writer, and returns a `HandshakeResult` containing the negotiated `Limits` and the cartridge's manifest bytes.
...
Reads the host's Hello, sends the cartridge's Hello (with manifest), negotiates limits, updates both reader and writer, and returns the negotiated `Limits`.

Source: `capdag/src/bifaci/io.rs`.

## Swift Equivalent

The Swift CartridgeRuntime in `capdag-objc/Sources/Bifaci/CartridgeRuntime.swift` follows the same handshake protocol. The `run()` method reads the host's Hello from stdin, sends the cartridge's Hello with manifest to stdout, and negotiates limits identically. The identity handler is registered automatically.
