...
|----------|-----------|-------------|
| `generate_thumbnail_urn(media)` | `generate;target=thumbnail` | Render a visual preview of the input. |
| `extract_metadata_urn(media)` | `extract;target=metadata` | Extract structured metadata (title, author, page count, dates). |
| `extract_outline_urn(media)` | `extract;target=outline` | Extract document outline or table of contents. |
...
Cap URN dispatch (see [../07-DISPATCH.md](../07-DISPATCH.md)) ensures that the incoming request matches exactly one of the registered media types. The handler's fallback loop over URNs finds the matching stream.
...
4. **Implement handlers**: Each handler implements `Op<()>`. See [16.2-HANDLER-PATTERNS.md](16.2-HANDLER-PATTERNS.md) for the standard patterns.
