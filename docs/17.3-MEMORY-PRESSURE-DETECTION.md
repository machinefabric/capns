...
How to detect and respond to memory pressure caused by cartridge processes
(inference model loads, large allocations) on macOS, before the system
becomes unresponsive.

## The Problem

When an inference cartridge loads a large model (8–14GB GGUF via llama.cpp,
...
1. It may kill the host app instead of the cartridge
2. It fires too late — the system is already frozen/unresponsive
3. It provides no notification to our code — the cartridge just dies
4. In production, it has been observed to fail entirely

We must detect pressure proactively and kill the offending cartridge ourselves.
...
if consecutiveSwapGrowth >= 3 {
    // Compressor overflowed — kill the cartridge NOW
    killCartridge()
}
```
...
checks for sustained swap growth. When triggered, kills inference cartridges
via the appropriate path (XPC cartridges via `cartridgeMonitor.killCartridge()`,
engine cartridges via `KillCartridges` gRPC RPC).
...
Fires before jetsam (when it fires at all). Triggers immediate cartridge
kill via `killInferenceCartridgesNow()`.

### Kill Routing

Cartridges are hosted in different contexts:
- **XPC cartridges**: killed via `cartridgeMonitor.killCartridge()` (XPC service
  is the parent process — kill succeeds)
- **Engine cartridges**: killed via `KillCartridges` gRPC RPC (engine is the
  parent — `CartridgeProcessHandle.kill_cartridge()` succeeds)
- **Direct kill()**: fails with EPERM for sandboxed/engine-hosted
  cartridges (sandbox blocks cross-process signals)
...
1. Spawning `testcartridge` as a cartridge via `CartridgeHost`
...
4. Killing the cartridge proactively
5. Verifying the cartridge is dead and the system recovered

**PASS condition**: We detected pressure and killed the cartridge before
jetsam did.

**FAIL condition**: Jetsam killed the cartridge before we detected — means
