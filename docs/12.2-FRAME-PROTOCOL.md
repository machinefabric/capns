...
```json
{
  ...
  "cap_groups": [
    {
      "name": "default",
      "caps": [{"urn": "cap:in=media:;out=media:", "title": "Identity", ...}, ...],
      "adapter_urns": []
    }
  ]
}
...
- `manifest` (bytes): JSON-encoded object with the structure:
  ...
  ```
  `caps` is a flat list of capability URN strings from all healthy cartridges on the host. `installed_cartridges` carries cartridge identity information. CAP_IDENTITY (`cap:in="media:";out="media:"`) must be present — the RelaySwitch rejects the connection otherwise.
