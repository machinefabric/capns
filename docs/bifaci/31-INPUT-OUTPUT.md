...
Source: `capdag/src/bifaci/plugin_runtime.rs:146`.
...
Source: `plugin_runtime.rs` (`find_stream`, `require_stream`, line 316).
...
These emit LOG frames that travel alongside data frames. See [33-PROGRESS-AND-LOGGING.md](33-PROGRESS-AND-LOGGING.md).
...
This matches the `json_value_to_bytes` convention used in the planner module (see [43-PLANNER.md](43-PLANNER.md)): strings become raw UTF-8 bytes (no JSON quoting), everything else becomes JSON-encoded bytes.

## Swift Equivalent

The Swift OutputStream and InputStream in `capdag-objc/Sources/Bifaci/PluginRuntime.swift` follow the same semantics with platform-appropriate types:
