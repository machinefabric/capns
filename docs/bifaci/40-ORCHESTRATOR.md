...
`validate_dag()` checks the graph for:

- **Cycles**: Uses topological sort (Kahn's algorithm). If the sort cannot complete (remaining nodes with unsatisfied dependencies), the graph contains a cycle.
- **Missing caps**: Each edge's cap URN must resolve in the registry.
- **Media conflicts**: Each node's media URN must be consistent across all edges connecting to it.
- **Structure mismatches**: Source structure (record vs opaque, from [../11-MEDIA-URNS.md](../11-MEDIA-URNS.md)) must be compatible with what the target cap expects.

Source: `capdag/src/orchestrator/validation.rs`.
