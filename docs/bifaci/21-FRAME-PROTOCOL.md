...
Payloads that exceed max_frame must be split into chunks (see [23-STREAMING.md](23-STREAMING.md)).
...
The meta map also carries limit proposals: `max_frame`, `max_chunk`, and `max_reorder_buffer`. Both sides negotiate limits by taking the minimum of the two proposals. See [22-HANDSHAKE.md](22-HANDSHAKE.md) for the full handshake sequence.
...
The `routing_id` (key 13) is assigned by the RelaySwitch at routing boundaries. Plugins sending peer invocations do not set routing_id — it is added by the infrastructure.
...
Health monitoring frame. Either side can send a Heartbeat; the receiver must respond with a Heartbeat carrying the same `id`. The PluginHostRuntime sends heartbeats every 30 seconds and expects a response within 10 seconds. A plugin that fails to respond is marked unhealthy.
...
Source: `frame.rs` (`Frame::relay_notify`); see also [36-RELAY-TOPOLOGY.md](36-RELAY-TOPOLOGY.md).
