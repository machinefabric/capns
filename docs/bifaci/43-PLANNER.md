...
This is different from the orchestrator (see [40-ORCHESTRATOR.md](40-ORCHESTRATOR.md)), which takes an explicit DAG description. The planner builds the DAG by searching the capability graph.
...
- **ResolvedArgument**: The final resolved value — concrete bytes ready to be sent to the plugin.
...
- **Bifaci plugins**: The production backend, routing through a RelaySwitch.
- **In-process handlers**: For testing with `InProcessPluginHost`.
