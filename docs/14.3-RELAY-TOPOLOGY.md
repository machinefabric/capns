...
- **manifest** (bytes): JSON-encoded aggregate manifest of all cartridges on the host. The payload has the structure:
  ```json
  {
  ...
  ```
  `caps` is a flat list of all capability URN strings from all healthy cartridges on the host. Every host must include `CAP_IDENTITY` (`cap:in="media:";out="media:"`) — the RelaySwitch rejects connections that do not advertise it.
