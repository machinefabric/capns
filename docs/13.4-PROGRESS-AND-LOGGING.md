...
How cartridges emit progress updates and log messages, and how these flow through the execution chain.

## LOG Frames

LOG frames are how cartridges communicate status to the engine and UI during request handling. They are non-terminal — a handler can emit LOG frames at any point between the first STREAM_START and the final END. LOG frames travel alongside data frames on the same request channel and interleave freely with CHUNK frames.
...
Source: `capdag/src/bifaci/cartridge_runtime.rs` (`OutputStream::progress`, `OutputStream::log`).
...
Keepalive frames solve a specific problem: blocking FFI calls (model loads) can starve the frame writer, causing the engine to think the cartridge is dead.

### The Problem

The cartridge's frame writer task runs on a tokio worker thread. It drains the output channel and writes frames to stdout. When a handler calls a blocking FFI function (e.g., `llama_model_load()` which can take minutes for large models), the call runs on the same tokio worker thread pool.
...
Source: `cartridge_runtime.rs` (`run_with_keepalive`, line 613).

### runWithKeepalive (Swift)

The Swift equivalent in `capdag-objc/Sources/Bifaci/CartridgeRuntime.swift` uses structured concurrency:
...
Source: `cartridge_runtime.rs` (`ProgressSender`, line 357; `progress_sender`, line 591).
...
Progress values pass through several layers between the cartridge and the UI:

```mermaid
graph TD
    PH["Cartridge handler<br/>output.progress(0.5, ...)"] --> LOG["LOG frame on stdout"]
    LOG --> PHR["CartridgeHostRuntime<br/>(pass-through)"]
...
1. **Cartridge handler**: Emits raw progress in [0.0, 1.0].
...
3. **execute_fanin**: The orchestrator's execution loop reads LOG frames from the response channel and calls the `CapProgressFn` callback with the cartridge's progress value.
...
See [15.3-PROGRESS-MAPPING.md](15.3-PROGRESS-MAPPING.md) for how `ProgressMapper` deterministically subdivides progress ranges across steps.

## Logging Restrictions

Cartridges do not have a tracing subscriber. The following do not work inside cartridge processes:
...
The only way to emit observable output from a cartridge is through LOG frames via `output.log()` and `output.progress()`. These send frames through the stdout channel, which the host receives and processes.
