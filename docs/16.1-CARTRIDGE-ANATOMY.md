...
The structure of a cartridge: entry point, manifest, cap definitions, and handler registration.

## What Is a Cartridge

A cartridge is a standalone binary that communicates with the engine via the capdag protocol over stdin/stdout. Each cartridge provides one or more capabilities (caps). The engine discovers cartridges, spawns them on demand when their caps are needed, and manages their lifecycle through the CartridgeHostRuntime.
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    runtime.register_op("cap:in=...;out=...;generate", || Box::new(GenerateOp));
    runtime.register_op("cap:in=...;out=...;extract;target=metadata", || Box::new(MetadataOp));
...
The `#[tokio::main]` attribute sets up the multi-threaded async runtime. `run()` detects whether the binary was launched with CLI arguments (CLI mode) or without (capdag cartridge mode) and handles both.
...
let manifest = CapManifest::new(
    ...
    vec![CapGroup::default_group(caps)],
);
```
...
- **cap_groups**: List of `CapGroup` instances, each containing a named set of `Cap` definitions and optional `adapter_urns`. Use `CapGroup::default_group(caps)` for a single unnamed group.
- **author** (optional): Maintainer name.
- **page_url** (optional): Documentation or repository URL.

The `cap_groups` structure exists to support adapter routing — a group can declare `adapter_urns` that identify which media adapters are available for that group's caps. For most cartridges, a single default group with no adapter URNs is correct.

Source: `capdag/src/bifaci/manifest.rs`, `capdag/src/cap/definition.rs`.

### Identity Cap

Every manifest must include the identity cap — `identity_cap()` in Rust or the equivalent in Swift. This cap handles the identity verification step during handshake (see [12.3-HANDSHAKE.md](12.3-HANDSHAKE.md)). `CartridgeRuntime::new()` panics if CAP_IDENTITY is missing from the manifest.
...
```rust
let cap = Cap::new(
    CapUrn::from_string("cap:in=\"media:pdf\";out=\"media:record\";extract;target=metadata").unwrap(),
...
Each cap in the manifest must have a corresponding handler registered with `CartridgeRuntime`. The cap URN string used for registration must match what dispatch will resolve — it must be `is_dispatchable` against incoming requests for the caps declared in the manifest.
...
runtime.register_op_type::<MetadataOp>("cap:in=...;out=...;extract;target=metadata");
```
...
1. **Discovery**: The engine's cartridge registry or dev_cartridges mapping identifies which cartridge binary provides a given cap.
2. **First request**: When a REQ arrives for a cap this cartridge provides, the CartridgeHostRuntime spawns the binary with no arguments (triggering cartridge CBOR mode).
3. **Handshake**: HELLO exchange and identity verification (see [12.3-HANDSHAKE.md](12.3-HANDSHAKE.md)).
