...
The `#[tokio::main]` attribute sets up the multi-threaded async runtime. `run()` detects whether the binary was launched with CLI arguments (CLI mode) or without (capdag plugin mode) and handles both.
...
Every manifest must include the identity cap — `identity_cap()` in Rust or the equivalent in Swift. This cap handles the identity verification step during handshake (see [12.3-HANDSHAKE.md](12.3-HANDSHAKE.md)). `PluginRuntime::new()` panics if CAP_IDENTITY is missing from the manifest.
