...
let response = req.peer().call_with_bytes(
    "cap:in=\"media:model-spec;textable\";out=\"media:file-path;textable\";download-model",
...
See [13.4-PROGRESS-AND-LOGGING.md](13.4-PROGRESS-AND-LOGGING.md) for the general progress forwarding pattern.
...
`run_with_keepalive` moves the blocking call to `tokio::task::spawn_blocking` (a separate thread pool) and emits progress frames every 30 seconds on the freed-up async runtime. See [13.4-PROGRESS-AND-LOGGING.md](13.4-PROGRESS-AND-LOGGING.md) for the mechanism.
