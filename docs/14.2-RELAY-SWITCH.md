...
5. Rebuild the `LiveCapFab` for path finding.
...
- `get_reachable_targets()` — queries the `LiveCapFab` to find which output media URNs are reachable from a given input.
