...
The `RelaySwitch` is the central routing component of the capdag network. It connects to one or more RelaySlave/RelayMaster pairs (each backed by a PluginHostRuntime managing one or more plugins) and routes requests to the correct master based on cap URN matching.
