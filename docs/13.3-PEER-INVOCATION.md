...
How a plugin calls another plugin's cap during request handling.

## Concept

A plugin handling a request can invoke a capability provided by a different plugin. The calling plugin does not know or care which plugin provides the target cap — the RelaySwitch handles routing based on cap URN matching.
...
Peer invocations happen during handler execution, not before or after. The handler is mid-request, needs something from another plugin, calls it, waits for the response, and continues.

Source: `capdag/src/bifaci/plugin_runtime.rs` (PeerInvoker trait, line 728).
...
let response = req.peer()
    .call_with_bytes("cap:in=...;out=...;op=download", &[
...
```rust
let call = req.peer().call("cap:in=...;out=...;op=something")?;
...
1. Plugin A sends REQ without XID — plugins don't know about routing IDs.
2. PluginHostRuntime A records the request's RID in `outgoing_rids` to know where to route the response.
3. The RelaySwitch assigns an XID and records the peer request in `peer_requests`.
4. LOG frames from Plugin B flow back through the same path. PluginHostRuntime A routes them to Plugin A's PeerResponse channel by matching the RID in `outgoing_rids`.
5. The response (CHUNK, END) follows the same routing path back to Plugin A.

Source: `host_runtime.rs` (`outgoing_rids`), `relay_switch.rs` (`peer_requests`), `plugin_runtime.rs`.
...
Source: `plugin_runtime.rs:753`.

## Swift Equivalent

The Swift implementation in `capdag-objc/Sources/Bifaci/PluginRuntime.swift` provides:
