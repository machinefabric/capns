...
The manifest must always include `identity_cap()` — the identity capability required for handshake verification (see [12.3-HANDSHAKE.md](12.3-HANDSHAKE.md)). `PluginRuntime::new()` panics if CAP_IDENTITY is missing.
...
This dual-mode design means a single binary serves as both a capdag plugin (when launched by the host with no args) and a standalone CLI tool (when run directly by a user).
...
- **`peer()`** → `&dyn PeerInvoker`: For calling other plugins' capabilities. See [13.3-PEER-INVOCATION.md](13.3-PEER-INVOCATION.md).
