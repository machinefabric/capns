...
Source: `capdag/src/bifaci/frame.rs` (StreamStart, StreamEnd); `cartridge_runtime.rs` (InputStream, OutputStream).
...
Source: `capdag/src/bifaci/io.rs` (`FrameWriter::write_chunked`); `cartridge_runtime.rs` (`OutputStream::write`).
