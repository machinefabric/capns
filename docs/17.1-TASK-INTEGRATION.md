...
2. **Resolve input files**: Maps file paths to container paths via `FileOperationCoordinator`. Source files in listings have macOS security bookmarks; the coordinator copies files to the cartridge container directory where cartridges can access them.
3. **Resolve argument values**: Reads `slot_values` and `cap_settings` from the task context. These are passed to the planner's slot resolution (see [15.4-PLANNER.md](15.4-PLANNER.md)).
...
2. **FileOperationCoordinator**: Copies files to the cartridge container directory. This directory is accessible to cartridge processes without additional entitlements.
3. **Container paths**: The copied file paths are passed to the DAG as input data for the first step's `InputSlot` nodes.

Security bookmarks are a macOS runtime concept — they are never serialized into the capdag protocol or into plans. The file resolution happens entirely in the machfab engine layer, before any frame reaches a cartridge.

Source: machfab `FileOperationCoordinator`, `cap_interpreter`.

## Progress Chain

The full progress chain from cartridge to UI:
...
1. **Cartridge handler**: `output.progress(0.5, "message")` sends a LOG frame.
...
- **Failure**: The error message is stored, task state → `Failed`. The `ExecutionError` variant determines the message (timeout, cartridge error, missing data, etc.).
